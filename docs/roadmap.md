...
This page lists work for the next AryaOS contributor. Items appear in priority order within each
group. Update this living document as items land. See [Agent handoff](agent-handoff.md) for the
...
- [ ] **Unified COP map on the portal.** `tar1090` shows only ADS-B. A portal
      ...
      (`/run/aryaos/neighbors.json` already exists) can make a device a
