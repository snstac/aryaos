...
!!! tip "Prefer site-wide TLS"
    In most deployments you install **one** set of certs once, on the [AryaOS Site page](./aryaos-site.md#site-wide-tak-tls-certificates). Every gateway inherits it. Use the per-gateway TLS card only when a single service needs a *different* client identity from the rest.
