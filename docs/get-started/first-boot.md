...
- [ ] **Rotate the Node-RED admin password.** AryaOS ships Node-RED with a publicly known default admin password. The Node-RED editor can run arbitrary code. Rotate it in **Cockpit > AryaOS Site > Node-RED admin password** (or run [`aryaos-set-nodered-password`](../reference/cli-helpers.md#aryaos-set-nodered-password)). See [Node-RED dashboard](../node-red.md).
