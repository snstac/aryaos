...
2. **Confirm the suffix.** The SSID is `AryaOS-xxxx` where `xxxx` matches the hostname. Look carefully - it can not be the name you expect. See [`DEVICE_SUFFIX`](reference/glossary.md#device_suffix).
...
If the box repeatedly reboots, an inadequate power supply is the likely cause. AryaOS enters
**safe mode** to remain reachable. Safe mode stops sensors and turns off USB peripherals.
