...
- **Live:** set `OWNSHIP_UID` to this box's GPSCOT/LINCOT CoT UID so the device's own GPS becomes ownship. See [Own position / GPS](./own-position-gps.md).
...
- [Air - ADS-B & UAT](./air-adsb.md) - feed the aircraft that GDLCOT rebroadcasts.
- [Own position / GPS](./own-position-gps.md) - supply GDLCOT's ownship.
